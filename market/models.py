...
class User(db.Model, UserMixin):
    ...
    bugdet = db.Column(db.Integer(), nullable=False, default=1000)
    items = db.relationship('Item', backref='owned_user', lazy=True)

    @property
    def prettier_budget(self):
        if len(str(self.bugdet)) >= 4:
            return f'{self.bugdet:,}'
        else:
            return f"{self.bugdet}"
    ...
    def can_purchase(self, item_obj):
        return self.bugdet >= item_obj.price
